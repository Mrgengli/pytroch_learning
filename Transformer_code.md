...
* nn.Embedding()函数在内部使用了一个查找表（lookup table）来进行映射。

* 具体来说，在嵌入层创建时，会初始化一个大小为num_embeddings × embedding_dim的嵌入矩阵，其中num_embeddings表示离散数据的数量，embedding_dim表示嵌入向量的维度。这个嵌入矩阵的每一行都对应一个离散数据对应的嵌入向量。当输入一个离散数据时，nn.Embedding()函数会根据这个数据的索引，在嵌入矩阵中查找对应的嵌入向量，并将其返回作为输出。

* 举个例子，假设我们有一个词汇表包含10个单词，嵌入向量的维度为5。那么nn.Embedding()函数会创建一个10 × 5的嵌入矩阵，每一行对应一个单词的嵌入向量。当输入一个单词的索引时（比如单词"apple"对应索引0），函数会查找嵌入矩阵中索引为0的行，返回这个行对应的嵌入向量作为输出。

* 嵌入层的使用可以有效地将离散数据转换为低维度的实数向量，从而方便神经网络的训练和处理。这种查找表的方法使得嵌入层的参数量相对较少，因为参数的数量只与离散数据的数量和嵌入向量的维度相关，而不依赖于输入数据的长度。
